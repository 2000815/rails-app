class Api::V1::InforController < ApplicationController
  def index
    ...
    unless latitude_portal_code[0] == "<"
      ...
      uri = URI.parse("https://webservice.recruit.co.jp/hotpepper/gourmet/v1/?key=#{ENV['API_KEY']}&lat=#{latitude_portal_code}&lng=#{longitude_portal_code}&range=#{range}&count=100&format=json")
  ...
  def show
    id = params[:id]
    client = HTTPClient.new
    uri = URI.parse("https://webservice.recruit.co.jp/hotpepper/gourmet/v1/?key=#{ENV['API_KEY']}&id=#{id}&format=json")
    response  = client.get(uri)
    res_json  = JSON.parse(response.body)
